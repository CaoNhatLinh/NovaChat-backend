...
public class UserPresenceResponse {
    ...
    public static UserPresenceResponse fromEntity(UserPresence presence) {
        ...
        return UserPresenceResponse.builder()
            ...
            .status(presence.getStatus()) // Already a string in Cassandra version
            .lastSeen(presence.getLastSeen())
            .isOnline(presence.isOnline())
            .lastActiveAgo(presence.isOnline() ? null : formatLastActive(presence.getLastSeen()))
            .build();
    }
    
    private static String formatLastActive(Instant lastSeen) {
        if (lastSeen == null) {
            return "Không rõ";
        }
        
        long secondsAgo = Instant.now().getEpochSecond() - lastSeen.getEpochSecond();
