...
public class ConversationController {
     ...
     * URL: PUT /api/conversations/{conversationId}
     */
     ...
     * URL: DELETE /api/conversations/{conversationId}/permanent
     */
    @DeleteMapping("/{conversationId}/permanent")
    public ResponseEntity<String> permanentDeleteConversation(
            @PathVariable UUID conversationId,
            Authentication authentication
    ) {
        try {
            ...
            if (deleted) {
                return ResponseEntity.ok("Conversation đã được xóa vĩnh viễn");
            } else {
                return ResponseEntity.notFound().build();
            }
        } catch (RuntimeException e) {
